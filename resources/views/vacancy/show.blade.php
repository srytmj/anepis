    ...
    <div class="tw-max-w-4xl tw-mx-auto tw-mt-10 tw-bg-white tw-rounded-lg tw-shadow-sm tw-p-6 relative">
        ...
        <div class="tw-grid tw-grid-cols-2 tw-items-center tw-mb-6">
            ...
            <div class="tw-text-right">
                <div class="tw-text-black tw-px-3 tw-py-1 tw-rounded">
                    Batas Melamar
                    {{ $vacancy->close_date ? \Carbon\Carbon::parse($vacancy->close_date)->format('d M Y') : '-' }}
                </div>
            </div>
        ...
        <div class="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4 tw-mb-6">
            ...
            <div class="tw-p-4 tw-bg-gray-100 tw-rounded">
                <p class="tw-font-semibold tw-mb-2">
                    <img src="{{ asset('images/people.png') }}" class="tw-inline tw-h-5 tw-mr-1" alt="Dosen"> Dosen
                    Pengampu:
                </p>
            ...
            <div class="tw-p-4 tw-bg-gray-100 tw-rounded">
                <p class="tw-font-semibold tw-mb-2">
                    <img src="{{ asset('images/people.png') }}" class="tw-inline tw-h-5 tw-mr-1" alt="Slot"> Slot /
                    Quota:
                </p>
            ...
            <div class="tw-p-4 tw-bg-gray-100 tw-rounded">
                <p class="tw-font-semibold tw-mb-2">
                    <img src="{{ asset('images/toga.png') }}" class="tw-inline tw-h-5 tw-mr-1" alt="Program Studi"> Program
                    Studi:
                </p>
            ...
            <div class="tw-p-4 tw-bg-gray-100 tw-rounded">
                <p class="tw-font-semibold tw-mb-2">
                    <img src="{{ asset('images/schedule.png') }}" class="tw-inline tw-h-5 tw-mr-1" alt="Jadwal"> Jadwal
                    Mata Kuliah:
                </p>
                @if ($vacancy->course->schedules->isNotEmpty())
                    <ul class="tw-list-disc tw-ml-5 tw-text-gray-700">
                        @foreach ($vacancy->course->schedules as $sch)
                            <li>{{ $sch->day }} | {{ \Carbon\Carbon::parse($sch->start_time)->format('H:i') }} -
                                {{ \Carbon\Carbon::parse($sch->end_time)->format('H:i') }}</li>
                        @endforeach
                    </ul>
                @else
        ...
        @auth
            @if (auth()->user()->role === 'student')
                @php
                    // Ambil student id dari foreignid
                    $studentId = auth()->user()->foreignid;

                    // Cek apakah student sudah apply vacancy ini
                ...
                <div class="tw-text-center tw-mb-6">
                    ...
                    @else
                        <form action="{{ route('vacancy.apply.store') }}" method="POST">
                            @csrf
                            <input type="hidden" name="vacancy_id" value="{{ $vacancy->id }}">
                            <input type="hidden" name="student_id" value="{{ $studentId }}">
                            <button type="submit"
                                class="tw-bg-red-600 tw-text-white tw-px-6 tw-py-2 tw-rounded hover:tw-bg-red-700">
                                Tambahkan Lamaran
                            </button>
                        </form>
                    @endif
